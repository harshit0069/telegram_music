...
from pytgcalls.types import AudioPiped  # Modern API
import yt_dlp
...
async def play(_, message: Message):
    if len(message.command) < 2:
        return await message.reply_text("⚠️ Usage: /play <YouTube URL>")
    file_path = await download_audio(" ".join(message.command[1:]))
    await message.reply_text(f"✅ Downloaded: {os.path.basename(file_path)}\nJoining VC...")
    await pytgcalls.join_group_call(GROUP_CHAT_ID, AudioPiped(file_path))
